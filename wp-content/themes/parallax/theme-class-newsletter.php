<?php
if ( ! class_exists( 'Themify_Newsletter' ) ) {
	/**
	 * Class to create newsletters
	 */
	class Themify_Newsletter extends Themify_Types {
		function shortcode($atts = array(), $post_type){
			...
			$posts = $newsletter_query->query(apply_filters('themify_'.$post_type.'_shortcode_args', $args));
			
			// Grid Style
			if( '' == $style ){
				$style = themify_check('setting-default_newsletter_index_post_layout')?
							 themify_get('setting-default_newsletter_index_post_layout'):
							 'grid4';
			}
			
			if( is_singular('newsletter') ){
				if( '' == $image_w ){
					$image_w = themify_check('setting-default_newsletter_single_image_post_width')?
							themify_get('setting-default_newsletter_single_image_post_width'):
							'670';
				}
				if( '' == $image_h ){
					$image_h = themify_check('setting-default_newsletter_single_image_post_height')?
							themify_get('setting-default_newsletter_single_image_post_height'):
							'0';
				}
				if( '' == $post_date ){
					$post_date = themify_check('setting-default_newsletter_single_post_date')?
							themify_get('setting-default_newsletter_index_post_date'):
							'yes';
				}
				if( '' == $title ){
					$title = themify_check('setting-default_newsletter_single_title')?
							themify_get('setting-default_newsletter_single_title'):
							'yes';
				}
				if( '' == $unlink_title ){
					$unlink_title = themify_check('setting-default_newsletter_single_unlink_post_title')?
							themify_get('setting-default_newsletter_single_unlink_post_title'):
							'no';
				}
				if( '' == $post_meta ){
					$post_meta = themify_check('setting-default_newsletter_single_meta')?
							themify_get('setting-default_newsletter_single_meta'):
							'yes';
				}
			} else {
				if( '' == $image_w ){
					$image_w = themify_check('setting-default_newsletter_index_image_post_width')?
							themify_get('setting-default_newsletter_index_image_post_width'):
							'222';
				}
				if( '' == $image_h ){
					$image_h = themify_check('setting-default_newsletter_index_image_post_height')?
							themify_get('setting-default_newsletter_index_image_post_height'):
							'175';
				}
				if( '' == $title ){
					$title = themify_check('setting-default_newsletter_index_title')?
							themify_get('setting-default_newsletter_index_title'):
							'yes';
				}
				if( '' == $unlink_title ){
					$unlink_title = themify_check('setting-default_newsletter_index_unlink_post_title')?
							themify_get('setting-default_newsletter_index_unlink_post_title'):
							'no';
				}
				// Reverse logic
				if( '' == $post_date ){
					$post_date = themify_check('setting-default_newsletter_index_post_date')?
							'no' == themify_get('setting-default_newsletter_index_post_date')?
								'yes' : 'no':
							'no';
				}
				if( '' == $post_meta ){
					$post_meta = themify_check('setting-default_newsletter_index_post_meta_category')?
							'no' == themify_get('setting-default_newsletter_index_post_meta_category')? 'yes' : 'no' :
							'no';
				}
			}
			...
			if( $posts ) {
				...
				$themify->newsletter_expander = $newsletter_expander;
				
				$out .= '<div class="loops-wrapper shortcode ' . $post_type  . ' ' . $style . ' '. $cpt_layout_class .'">';
					$out .= themify_get_shortcode_template($posts, 'includes/loop-newsletter', 'index');
...
if(!function_exists('themify_default_newsletter_single_layout')){
	/**
	 * Default Single Newsletter Layout
	 * @param array $data
	 * @return string
	 */
	function themify_default_newsletter_single_layout($data=array()){
		/**
		 ...
		 * @var string
		 */
		$prefix = 'setting-default_newsletter_single_';
		/**
		 ...
		 */
		$output = '<p>
						<span class="label">' . __('Hide Newsletter Title', 'themify') . '</span>
		...
		$output .=	'<p>
						<span class="label">' . __('Unlink Newsletter Title', 'themify') . '</span>
		...
		// Hide Post Meta /////////////////////////////////////////
		$output .=	'<p>
						<span class="label">' . __('Hide Newsletter Meta', 'themify') . '</span>
		...
		$output .=	'<p>
						<span class="label">' . __('Hide Newsletter Date', 'themify') . '</span>
		...
		$prefix = 'setting-newsletter_nav_';
		$output .= '
			<p>
				<span class="label">' . __('Newsletter Navigation', 'themify') . '</span>
				<label for="'. $prefix .'disable">
					<input type="checkbox" id="'. $prefix .'disable" name="'. $prefix .'disable" '. checked( themify_get( $prefix.'disable' ), 'on', false ) .'/> ' . __('Remove Newsletter Navigation', 'themify') . '
				</label>
				<span class="pushlabel vertical-grouped">
				<label for="'. $prefix .'same_cat">
					<input type="checkbox" id="'. $prefix .'same_cat" name="'. $prefix .'same_cat" '. checked( themify_get( $prefix. 'same_cat' ), 'on', false ) .'/> ' . __('Show only newsletters in the same category', 'themify') . '
				</label>
				</span>
			</p>';
...
if(!function_exists('themify_default_newsletter_index_layout')){
	/**
	 * Default Index Newsletter Layout
	 * @param array $data
	 * @return string
	 */
	function themify_default_newsletter_index_layout($data=array()){
		/**
		 * Associative array containing theme settings
		 * @var array
		 */
		 ...
		 * @var string
		 */
		$prefix = 'setting-default_newsletter_index_';
		/**
		 * Basic default options '', 'yes', 'no'
		 * @var array
		 */
		 ...
		 */
		$output = '<p>
						<span class="label">' . __('Newsletter Sidebar Option', 'themify') . '</span>';
		 ...
		 */
		$output .= '<p>
						<span class="label">' . __('Newsletter Layout', 'themify') . '</span>';
		...
		$output .=	'<p>
						<span class="label">' . __('Newsletter Expand', 'themify') . '</span>
						<label for="'.$prefix.'disable_porto_expand">
							<input type="checkbox" id="'.$prefix.'disable_porto_expand" name="'.$prefix.'disable_porto_expand" '. checked(isset( $data[$prefix.'disable_porto_expand'] )? $data[$prefix.'disable_porto_expand'] : '', 'on', false) .'/> ' . __('Check here to disable Newsletter expand', 'themify') . '
						</label>
					</p>';
...
if(!function_exists('themify_newsletter_slug')){
	/**
	 * Newsletter Slug
	 * @param array $data
	 * @return string
	 */
	function themify_newsletter_slug($data=array()){
		$data = themify_get_data();
		$newsletter_slug = isset($data['themify_newsletter_slug'])? $data['themify_newsletter_slug']: apply_filters('themify_newsletter_rewrite', 'project');
		return '
			<p>
				<span class="label">' . __('Newsletter Base Slug', 'themify') . '</span>
				<input type="text" name="themify_newsletter_slug" value="'.$newsletter_slug.'" class="slug-rewrite">
				<br />
...
if(!function_exists('themify_gallery_slider')){
	...
	function themify_gallery_slider(){
		 ...
		 * @var string
		 */
		$prefix = 'setting-newsletter_slider_';
		/**
		 * Basic default options '', 'yes', 'no'
		 * @var array
		 */
...
/**
 * Add extra Newsletter module options specific to Parallax theme
 *
 * @since 1.7.9
 * @return array
 */
function themify_parallax_newsletter_module_options( $options, $module ) {
	if( $module->slug == 'newsletter' ) {
		...
		$options[] = array(
			'id' => 'newsletter_expander',
			'type' => 'select',
			'label' => __('Enable Newsletter Expander', 'themify'),
			'options' => array(
				'yes' => __('Yes', 'themify'),
				'no' => __('No', 'themify'),
			)
...
add_filter( 'themify_builder_module_settings_fields', 'themify_parallax_newsletter_module_options', 10, 2 );